...
//			generateSeq(targetX, targetY, seq);
	...
	private void reasoning(boolean breeze, boolean stench, boolean scream) {
		...
		if(stench && hasWumpus) {
			...
			for(int i = 0; i < 7; i++) {
				for(int j = 0; j < 7; j++) {
					if(!((i == agentX-1 && j == agentY) || (i == agentX+1 && j == agentY) || (i == agentX && j == agentY-1) || (i == agentX && j == agentY+1))) {
						memBoard[i][j].wumpus = false;
						continue;
					} else if(valid(i, j) && memBoard[i][j].wumpus) {
						wumpus_pos++;
						targetX = i;
						targetY = j;
					}
			...
			if(wumpus_pos == 1 && hasArrow) {
				seq.clear();
				seq.push(Action.SHOOT);
				if(targetX < agentX) {
					if(agentDir > 2)
						seq.push(Action.TURN_LEFT);
					else {
						for(int k = 0; k < 2 - agentDir; k++)
							seq.push(Action.TURN_RIGHT);
					}
				} else if(targetX > agentX) {
					if(agentDir == 3)
						seq.push(Action.TURN_RIGHT);
					else {
						for(int k = agentDir; k > 0; k--)
							seq.push(Action.TURN_LEFT);
					}
